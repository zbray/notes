...
`grid-template-columns` The number of values dictates the number of columns.
...
`grid-template-rows` defines the way the rows look.
...
`grid-gap`  Dictates the space between grids and other elements
